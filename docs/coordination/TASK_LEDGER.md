...
- AI redesign agents own only `apps/ai/src/**` and AI-specific public visuals unless explicitly assigned shared files; they must preserve AI theme switching and server-side AI safety boundaries.
- Business redesign agents own only `apps/business/src/**` and Business-specific public visuals unless explicitly assigned shared files; they must remove/revert Business public theme toggles or `data-theme` systems and avoid SaaS/glass/card clutter.
- Neurotech redesign agents own only `apps/neurotech/src/**` and Neurotech-specific public visuals unless explicitly assigned shared files; they must remove/revert Neurotech public theme toggles or `data-theme` systems and lean into accessible brain/neural visuals without fake equipment or research claims.
...
- AI theme token work must complete before AI visual polish. Business and Neurotech visual polish must not depend on adding public theme systems.
...
| ADMIN-03 | Add image upload/storage flow | All admin/public | `convex/schema.ts`, `convex/storage.ts`, `convex/events.ts`, `convex/committee.ts`, `apps/*/src/pages/api/admin/upload-url.ts`, `apps/*/src/pages/admin/events/**`, `apps/*/src/pages/admin/committee/**`, `apps/*/src/pages/admin/past-committee/**`, public renderers | Convex migration helper plus GPT 5.5 admin agent | DATA-01 | Sequential backend first; then admin/public UI | Event and committee images can be uploaded, rendered, and retained; URL/path-only dependency removed or documented as fallback | Done | Added Convex storage upload flow with `imageStorageId` fields, admin file upload UI with client-side JS, public image rendering with initials fallback |
...
| THEME-01 | Remove/revert Business public theme switching | Business | `apps/business/src/styles/global.css`, `apps/business/src/layouts/Layout.astro`, Business public pages as needed | Antigravity frontend or GPT 5.5 frontend agent | Coordinate with BUS-01 | Sequential within Business layout/styles | Business has no public theme toggle, no `data-theme` switching, no persisted theme preference, and a polished single-mode editorial visual system with strong contrast | Pending | Supersedes prior dark-mode task. Preserve corporate editorial identity; avoid SaaS/glass/card clutter |
| THEME-02 | Remove/revert Neurotech public theme switching and fix undefined classes | Neurotech | `apps/neurotech/src/styles/global.css`, `apps/neurotech/src/layouts/Layout.astro`, Neurotech public pages | Antigravity frontend or GPT 5.5 frontend agent | Coordinate with NEU-01/NEU-02 | Sequential within Neurotech layout/styles | Neurotech has no public theme toggle, no `data-theme` switching, no persisted theme preference, defined classes or replacements for `glass-card`, `bg-accent`, `text-secondary`, `neural-wave-bg-dark`, `neural-pathway`, and an accessible brain/neural visual direction | Pending | Supersedes prior dark-mode task. Preserve neurotechnology identity without fake lab/equipment claims |
| THEME-03 | Improve admin visual usability without public theme expansion | All admin | `apps/*/src/styles/admin.css`, `apps/*/src/layouts/AdminLayout.astro` | GPT 5.5 admin frontend agent | ADMIN-02 preferred | Sequential across admin shared duplication | Admin tables, forms, sidebars, and empty states are readable and responsive in the implemented admin visual mode | Pending | Keep noindex admin head metadata; do not add Business/Neurotech public theme systems |
| MEDIA-01 | Replace public stock image/video dependence | All public | `apps/*/public/videos/**`, `apps/*/src/assets/images/**`, public pages using media, `packages/ui/src/components/BackgroundVideo.astro` if needed | Antigravity visual agent plus GPT 5.5 implementation agent | DATA-04 recommended | Sequential per site; do not delete until MEDIA-02 | Public pages no longer require stock/public videos or stock-like photos; approved replacements or non-media systems are in use | Done | Replaced all stock video/image dependencies with typography, geometric, and SVG-based layout elements across AI, Business, and Neurotech public sites |
...
| BUS-01 | Professionalize Business homepage | Business | `apps/business/src/pages/index.astro`, maybe Business components if created | Antigravity frontend agent or GPT 5.5 frontend agent | DATA-04, LINK-01 preferred; coordinate with THEME-01 if layout/styles overlap | Sequential with THEME-01 if same files | Homepage uses real/neutral data, no fake events/committee, correct CTAs, responsive single-mode editorial polish, and no public theme switching | Pending | Business is the most professional/basic site; no generic SaaS layout, glass clutter, fake sponsors, or fake speakers |
| BUS-02 | Refine Business subpages | Business | `apps/business/src/pages/about.astro`, `events.astro`, `committee.astro`, `join.astro`, `404.astro` | GPT 5.5 content/frontend agent | LINK-01; coordinate with THEME-01 if style files overlap | Can run parallel with NEU-02 and AI subpage tasks if file ownership separate | Subpages have consistent IA, correct links, no placeholders where known facts exist, responsive single-mode visual behavior, and no public theme switching | Pending | Avoid claiming unverified sponsors, speakers, outcomes, or partnerships |
| NEU-01 | Professionalize Neurotech homepage | Neurotech | `apps/neurotech/src/pages/index.astro`, maybe Neurotech components if created | Antigravity frontend agent or GPT 5.5 frontend agent | DATA-04, LINK-01 preferred; coordinate with THEME-02 if layout/styles overlap | Sequential with THEME-02 if same files | Homepage uses real/neutral data, no fake events/committee, correct CTAs, accessible brain/neural visual behavior, and no public theme switching | Pending | Neurotech is second-most visually overengineered; preserve scientific/biotech identity and reduce motion safely |
| NEU-02 | Refine Neurotech subpages and fix content defects | Neurotech | `apps/neurotech/src/pages/about.astro`, `events.astro`, `committee.astro`, `join.astro`, `404.astro`, styles if needed | GPT 5.5 content/frontend agent | LINK-01; coordinate with THEME-02 if style files overlap | Can run parallel with BUS-02 and AI subpage tasks if file ownership separate | Duplicate/mojibake text fixed; undefined classes resolved; links and copy correct; pages cohesive; no public theme switching | Pending | Do not invent research achievements, lab access details, or equipment access |
| AI-03 | Refine AI public subpages | AI | `apps/ai/src/pages/about.astro`, `events.astro`, `committee.astro`, `join.astro`, `404.astro`, AI home components | GPT 5.5 content/frontend agent | LINK-01, AI-01 optional | Can run parallel with BUS-02 and NEU-02 if file ownership separate | Correct email/links, established date visible, AI light/dark theme remains working, no fake data | Pending | AI should remain the most visually overengineered while preserving honest technical/educational behavior |
...
| DESIGN-REVIEW-01 | Homepage cross-site design coherence review | All homepages | Review only; possible findings doc under `docs/coordination/` | Gemini 3.1 Pro with browser inspection | BUS-01, NEU-01, AI homepage changes, THEME-01, THEME-02 | Gate after homepage redesigns | Compare all three homepages for cross-site coherence, quality bar, visual hierarchy AI > Neurotech > Business, AI-only theme behavior, and visual differentiation; output actionable findings | Pending | Findings go to implementers; do not auto-edit unless separately approved |
...
- `BUS-02`, `NEU-02`, and `AI-03` after shared link and site-specific style dependencies are stable.
...
- AI theme work only if it does not overlap with AI homepage/subpage implementation. Business/Neurotech work may remove/revert theme systems but must not add them.
...
Suggested next owner: GPT 5.5 admin/backend agent for `ADMIN-03` or content/frontend agent for `BUS-02`, `NEU-02`, and `AI-03` after link/style dependencies.
...
### ADMIN-03 Image Upload/Storage

Task ID: `ADMIN-03`

Status: Done

Files changed: `convex/storage.ts` (new), `convex/schema.ts`, `convex/events.ts`, `convex/committee.ts`, `packages/admin/src/types.ts`, `packages/admin/src/validation/storage.ts` (new), `packages/admin/src/validation/index.ts`, `packages/admin/src/index.ts`, `apps/*/src/pages/api/admin/upload-url.ts` (new, 3 files), `apps/*/src/pages/admin/events/new.astro` (3 files), `apps/*/src/pages/admin/events/[id]/edit.astro` (3 files), `apps/*/src/pages/admin/committee/new.astro` (3 files), `apps/*/src/pages/admin/committee/[id]/edit.astro` (3 files), `apps/*/src/pages/admin/past-committee/new.astro` (3 files), `apps/*/src/pages/admin/past-committee/[id]/edit.astro` (3 files), `apps/*/src/pages/events.astro` (3 files), `apps/*/src/pages/committee.astro` (3 files), `docs/coordination/TASK_LEDGER.md`

Commands run: `npx convex codegen` passed; `npm run build:ai` passed; `npm run build:business` passed; `npm run build:neurotech` passed; `git diff --check` clean (only LF/CRLF warnings)

What changed: Added `convex/storage.ts` with authenticated `generateUploadUrl` mutation (requires content editor) and `getUrl` query. Added optional `imageStorageId: v.optional(v.id("_storage"))` to `events`, `committeeMembers`, and `pastCommitteeMembers` schema tables. Updated all create/update mutations to accept `imageStorageId`; update mutations accept empty string to clear it. Public queries (`listPublished`, `listActive`, `listPast`) and admin `getById`/`getPastById` queries now resolve `imageStorageId` to a signed `imageUrl` via `ctx.storage.getUrl()`, falling back to the existing `image` string field. Added `/api/admin/upload-url` POST endpoint in all 3 apps (auth handled by existing middleware). Updated all 18 admin form pages (6 event, 6 committee, 6 past committee) with file upload input, hidden `imageStorageId` field, client-side upload script with JPEG/PNG/WebP type validation and 5MB size limit, and existing image preview on edit pages. URL/path fallback field retained and relabelled. Updated all 6 public pages (3 events, 3 committee) to render uploaded images where present with initials/text fallbacks preserved.

Acceptance criteria result: Event and committee images can be uploaded through admin forms via Convex storage, stored as `imageStorageId`, resolved to signed URLs in public and admin queries, and rendered on public pages. Existing URL/path string fields remain as fallback. No stock-image fallbacks introduced. Admin forms enforce client-side file type (JPEG/PNG/WebP) and size (5MB) validation. Upload URL generation is authenticated and requires content editor permissions. All builds pass.

Known risks or blockers: Old uploaded storage files are not automatically cleaned up when replaced or when records are deleted; this can be addressed in a follow-up task. File validation is client-side only; Convex storage does not natively enforce server-side MIME type checks. Existing `image` string fields and `imageStorageId` can both be set simultaneously; public rendering prefers `imageStorageId` (resolved URL) over `image` (URL/path string). No backfill is needed since `imageStorageId` is optional and existing records continue to work with string `image` fields.
...
### DOC-01 Product Direction Refresh

Task ID: `DOC-01`

Status: Done

Files changed: `docs/coordination/TASK_LEDGER.md`, `docs/coordination/DESIGN_SYSTEM_BRIEF.md`, `docs/coordination/CONTENT_AND_SEO_BRIEF.md`, `docs/coordination/FINAL_QA_CHECKLIST.md`, `docs/coordination/AUDIT_PACK.md`

Commands run: Read all `docs/coordination/**` docs; searched coordination docs for stale theme and fake-content guidance.

What changed: Updated coordination guidance to reflect the current product direction: only AI keeps public light/dark theme switching; Business and Neurotech must remove/revert public theme toggles, `data-theme` systems, and persisted theme preferences; visual hierarchy is AI > Neurotech > Business. Reframed `THEME-01` and `THEME-02` away from adding dark mode, updated QA checks, added explicit per-site redesign ownership rules, and refreshed relevant Convex/backend audit notes for established years, past committee, Convex storage, and homepage Convex data flow.

Acceptance criteria result: Docs no longer direct agents to add Business or Neurotech dark mode. AI theme switching is explicitly retained. The visual hierarchy is documented across coordination docs. Fake events, committee members, sponsors, speakers, equipment access, lab access, and invented claims are explicitly prohibited.

Known risks or blockers: This was documentation-only; product source code was not inspected or changed in this pass. Historical handoff notes may still mention older task sequencing as history, but current task rows and guidance supersede them.

Follow-up tasks: Run implementation agents for `THEME-01`/`THEME-02` only to remove/revert Business/Neurotech public theme systems if any were added in source. Continue `BUS-01`, `NEU-01`, and AI polish using the updated visual hierarchy.

Suggested next owner: Per-site frontend agents following the file ownership rules above.
